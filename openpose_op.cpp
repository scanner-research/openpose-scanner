...
const int TOTAL_KEYPOINTS = POSE_KEYPOINTS + FACE_KEYPOINTS + 2 * HAND_KEYPOINTS;
...
public:
  ...
  void execute(const scanner::BatchedColumns& input_columns,
    ...
    for (auto& datum : *datumProcessed) {
      ...
      scanner::memcpy_buffer((scanner::u8*) gpu_kp, device_, (scanner::u8*) kp, scanner::CPU_DEVICE,
                             size);
...
REGISTER_KERNEL(OpenPose, OpenPoseKernel).device(scanner::DeviceType::GPU).num_devices(1).batch();
